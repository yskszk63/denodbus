...
function endianToNumber(endian: Endian): number {
  switch (endian) {
    case LITTLE_ENDIAN: return 0x6c;
    case BIG_ENDIAN: return 0x42;
  }
}

function numberToEndian(val: number): Endian {
  switch (val) {
    case 0x6c: return LITTLE_ENDIAN;
    case 0x42: return BIG_ENDIAN;
    default: throw new Error(`unknown endian ${val}`);
  }
}

function isMessageType(val: unknown): val is MessageType {
  switch (val) {
    case METHOD_CALL: return true;
    case METHOD_RETURN: return true;
...
function numberToFlags(val: number): Set<Flags> {
  if ((val & (0xFF ^ (NO_REPLY_EXPECTED | NO_AUTO_START | ALLOW_INTERACTIVE_AUTHORIZATION))) !== 0) {
    throw new Error(`unknown flag contained. ${val}`);
  }

  const r = new Set<Flags>();

  for (const f of [NO_REPLY_EXPECTED, NO_AUTO_START, ALLOW_INTERACTIVE_AUTHORIZATION] as Flags[]) {
    if (val & f) {
      r.add(f);
    }
  }
...
function checkHeader([name, val]: [number, t.Variant<any>]): [FieldName, t.Variant<any>] {
  switch (name) {
    case PATH: return [name, val];
    case INTERFACE: return [name, val];
    case MEMBER: return [name, val];
    case ERROR_NAME: return [name, val];
    case REPLY_SERIAL: return [name, val];
    case DESTINATION: return [name, val];
    case SENDER: return [name, val];
    case SIGNATURE: return [name, val];
    case UNIX_FDS: return [name, val];
    default: throw new Error(`unknown header ${name}`);
  }
}
...
export class Message {
  ...
  async marshall(stream: WritableStream<Uint8Array>): Promise<void> {
    ...
    try {
      ...
      await headerty.marshall(ctx, this.headers);
      // TODO body
    } finally {
      ctx.release();
    }
  ...
  static async unmarshall(stream: ReadableStream<Uint8Array>): Promise<Message> {
    ...
    try {
      ...
      const headers = await headerty.unmarshall(ctx).then(a => a.map(checkHeader));

      // TODO body
      return new Message(endian, type, flags, protocolVersion, serial, headers, []);
