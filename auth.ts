...
function encode(command: ClientToServerCommand): string {
  switch (command.command) {
    case "AUTH":
      return `AUTH ${command.mechanism} ${toHex(command.initialResponse)}`;
    case "CANCEL":
      return "CANCEL";
    case "BEGIN":
      return "BEGIN";
    case "DATA":
      return `DATA ${toHex(command.data)}`;
    case "ERROR":
      return `ERROR ${command.explanation}`;
...
function decode(text: string): ServerToClientCommand {
  ...
  switch (command) {
    case "REJECTED":
      return { command: "REJECTED", mechanisms: rest.split(" ") };
    case "OK":
      return { command: "OK", guid: fromHex(rest) };
    case "DATA":
      return { command: "DATA", data: fromHex(rest) };
    case "ERROR":
      return { command: "ERROR", explanation: rest };
    case "AGREE_UNIX_FD":
      return { command: "AGREE_UNIX_FD" };
    default:
  ...
  while (true) {
    ...
    if (buf.at(-2) === 0x0d && buf.at(-1) === 0x0a) {
      return new TextDecoder().decode(
        Uint8Array.from(buf.slice(0, buf.length - 2)),
      );
    }
  }
}
