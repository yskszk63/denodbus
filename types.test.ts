...
function memStream(mem: Uint8Array[]): [ReadableStream<Uint8Array>, WritableStream<Uint8Array>] {
...
Deno.test('byte', async () => {
  ...
  for (const v of [0, 1, 192, 255]) {
    ...
    await ty.marshall(v, w.getWriter());
    const unmarshalled = await ty.unmarshall(r.getReader({ mode: 'byob' }));
